...
test("typing requires correcting an error", () => { const session = new TypingSession("abc"); session.type("a", 0); session.type("x", 1000); session.type("b", 2000); assert.equal(session.index, 1); assert.equal(session.errorChar, "b"); session.backspace(); session.type("b", 3000); assert.equal(session.index, 2); assert.equal(session.errors, 2); });
...
test("manifest stages preserve valid lane geometry and scale past twenty tracks", () => {
  ...
  const legacyTracks = manifest.tracks.filter((track) => track.id !== "prehistoric-canyon");
  const canyon = manifest.tracks.find((track) => track.id === "prehistoric-canyon");
  assert.ok(legacyTracks.every((track) => JSON.stringify(track.laneBaselines) === JSON.stringify([443.25, 415.5, 387.75, 360, 471])));
  assert.deepEqual(canyon.laneBaselines, [484, 461, 438, 415, 507]);
  ...
  assert.ok(legacyTracks.every((track) => JSON.stringify(track.raceLine) === JSON.stringify({ startX: 114, finishX: 1977 })));
  assert.deepEqual(canyon.raceLine, { startX: 125, finishX: 1897 });
  assert.ok(legacyTracks.every((track) => track.laneBaselines[4] === 471));
  assert.equal(canyon.laneBaselines[4], 507);
  ...
  assert.ok(legacyTracks.every((track) => JSON.stringify(track.lanePaths[0]) === JSON.stringify({ start: { x: 136.75, y: 443.25 }, finish: { x: 1957.75, y: 443.25 } })));
  assert.deepEqual(canyon.lanePaths[0], { start: { x: 145, y: 484 }, finish: { x: 1884, y: 484 } });
  assert.ok(legacyTracks.every((track) => JSON.stringify(track.lanePaths[3]) === JSON.stringify({ start: { x: 205, y: 360 }, finish: { x: 1900, y: 360 } })));
  assert.deepEqual(canyon.lanePaths[3], { start: { x: 205, y: 415 }, finish: { x: 1845, y: 415 } });
  assert.ok(legacyTracks.every((track) => JSON.stringify(track.lanePaths[4]) === JSON.stringify({ start: { x: 114, y: 471 }, finish: { x: 1977, y: 471 } })));
  assert.deepEqual(canyon.lanePaths[4], { start: { x: 125, y: 507 }, finish: { x: 1897, y: 507 } });
